...
const int SDIO = 4;
const int LED = 10;
const int WINDOW = 5;                    // same as your Python rolling window
const float CONTACT_THRESH = 50;        // example
const float SLIP_THRESH_MOUSE = 0.3;     // example


const byte noSleep = 0xA0;
bool logStart = false;
float contactBuffer[WINDOW];
float mvtBuffer[WINDOW];
int contactIndex = 0;
int mvtIndex = 0;
...
bool slip_detection(uint8_t contact_raw, int8_t dx, int8_t dy) {

    // ---------- CONTACT ROLLING MEAN ----------
    // remove oldest
    contactSum -= contactBuffer[contactIndex];
    // add new
    contactBuffer[contactIndex] = contact_raw;
    contactSum += contact_raw;
    
    // ---------- MOVEMENT MAGNITUDE ----------
    float mvt = sqrt(dx*dx + dy*dy);

    mvtSum -= mvtBuffer[mvtIndex];
    mvtBuffer[mvtIndex] = mvt;
    mvtSum += mvt;

    // advance circular index
    contactIndex = (contactIndex + 1) % WINDOW;
    mvtIndex = (mvtIndex + 1) % WINDOW;

    // keep track of how many valid samples we have
    if (sampleCount < WINDOW) sampleCount++;

    // compute rolling means
    float contactMean = contactSum / sampleCount;
    float mvtMean = mvtSum / sampleCount;

    // apply thresholds
    bool contact = (contactMean > CONTACT_THRESH);
    bool slip_mvt = (mvtMean > SLIP_THRESH_MOUSE);

    // final slip decision
    bool slip = slip_mvt && contact;
