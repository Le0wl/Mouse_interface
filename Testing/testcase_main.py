...
def main():
    ...
    filename = os.path.join(SAVE_PATH, f"sensor_log_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}{SURFACE}.csv")
    filename2 = os.path.join(SAVE_PATH, f"robot_log_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}{SURFACE}.csv")
    ...
    # log go
    log_thread.start()
    if(log_ready_event.wait(timeout=10)): # waiting for logging set up /timeout at 10 secs 
        if thread_robo:
        ...
        if ('start_time'in timing) and thread_robo:
            ...
            if PLOT and os.path.exists(filename) and os.path.exists(filename2):
                plot_hist_sensor_robot(filename, filename2)
    else: 
